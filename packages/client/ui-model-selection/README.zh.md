...
- **搜索仅在 composer 面板内**——`/model` popupSelect 保持完整不过滤的选项列表；在共享弹层内过滤需要本包不拥有的 command-UI 扩展点。
